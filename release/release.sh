...
for os in "${osList[@]}"; do
    if [ "$os" = "windows" ]; then
        for arch in "${archList[@]}"; do
            ...
            cd ../ || exit
            env GOOS="$os" GOARCH="$arch" CGO_ENABLED=0 go build -o "./release/dist/$projectName-$os-$version-$arch/hpf.exe" main.go
            cd ./release || exit
        ...
        for arch in "${archList[@]}"; do
            ...
            mkdir "./dist/$projectName-$os-$version-$arch"
            cd ../ || exit
            env GOOS="$os" GOARCH="$arch" CGO_ENABLED=0 go build -o "./release/dist/$projectName-$os-$version-$arch/hpf" main.go
            cd ./release || exit
