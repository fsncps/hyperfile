...
func (m *model) treePanelRender(idx int) string {
	...
	for i := tree.renderIdx; i < end; i++ {
		...
		hasKids := node.isDir && tree.HasChildren(node.path)
		isVisuallyExpanded := hasKids && (node.depth < tree.maxDepth || tree.expanded[node.path]) && tree.IsExpanded(node.path)
		...
		// Style for expand indicator: branch color, faint for expanded state
		expandStyle := branchStyle.Background(bgColor)
		if isVisuallyExpanded {
			expandStyle = expandStyle.Faint(true)
		}
		bgStyle := lipgloss.NewStyle().Background(bgColor)
		line := branchStyle.Render(branchStr) +
			expandStyle.Render(expandIndicator) +
			bgStyle.Render(" ") + rendered
		if isCursorRow {
			// Pad to full content width so background fills the line
			lineW := ansi.StringWidth(line)
			cw := r.ContentWidth()
			if lineW < cw {
				line += bgStyle.Render(strings.Repeat(" ", cw-lineW))
			}
		}
