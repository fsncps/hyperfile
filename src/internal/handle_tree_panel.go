...
import (
	...
	"github.com/fsncps/hyperfile/src/internal/ui/logpanel"
...
const previewDebounceDuration = 100 * time.Millisecond //nolint:unused

type previewTickMsg struct{} //nolint:unused

//nolint:unused
func (m *model) focusNextPanel() {
	if m.activeFileArea == primaryPanelActive && m.secondaryPanel.open {
		m.setSecondaryPanelActive()
	} else {
		m.setPrimaryPanelActive()
	}
