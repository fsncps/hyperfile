...
func (m *model) secondaryPanelRender(width, height int) string {
	sp := &m.secondaryPanel
	...
	case SecondaryTree:
		...
		return m.treePanelRender(1)
	case SecondaryDetail:
		if sp.detail == nil {
			return ""
		}
		return m.renderDetailPanel(sp.detail, width, height)
	case SecondaryGit:
		if sp.git == nil {
			return ""
		}
		return sp.git.Render(width, height)
	case SecondaryDiskUsage:
		if sp.diskUsage == nil {
			return ""
		}
		return sp.diskUsage.Render(width, height)
	default:
		return ""
	}
}

func (m *model) renderDetailPanel(d *detailViewModel, width, height int) string {
	if d == nil || len(d.entries) == 0 {
		return common.FilePanelNoneText
	}

	r := ui.FilePanelRenderer(height+2, width+2, m.secondaryPanel.focusType == focus)
	visibleH := height - 2
	end := min(d.renderIdx+visibleH, len(d.entries))
	cw := r.ContentWidth()
	const fixedCols = 32
	nameWidth := cw - fixedCols
	if nameWidth < 4 {
		nameWidth = 4
	}

	for i := d.renderIdx; i < end; i++ {
		e := d.entries[i]
		isCursorRow := i == d.cursor
